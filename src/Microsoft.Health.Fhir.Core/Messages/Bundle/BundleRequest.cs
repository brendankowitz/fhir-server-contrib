...
public class BundleRequest : IRequest<BundleResponse>, IRequest, IRequireCapability
{
    ...
    {
        string bundleType = Bundle.Scalar<string>(KnownFhirPaths.BundleType);

        if (string.IsNullOrEmpty(bundleType)) throw new MethodNotAllowedException(Resources.TypeNotPresent);
