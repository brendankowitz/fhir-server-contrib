...
public static class RawResourceElementExtensions
{
    ...
    public static async Task SerializeToStreamAsUtf8Json(this RawResourceElement rawResource, Stream outputStream)
    {
        ...
        JsonElement.ObjectEnumerator enumerator = jsonDocument.RootElement.EnumerateObject();
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (current.NameEquals("meta"))
            {
                ...
                foreach (JsonProperty metaEntry in current.Value.EnumerateObject())
                ...
                if (!lastUpdatedFound && rawResource.LastUpdated.HasValue) writer.WriteString("lastUpdated", rawResource.LastUpdated.Value.ToInstantString());

                if (!versionIdFound) writer.WriteString("versionId", rawResource.VersionId);
        ...
        while (enumerator.MoveNext()) enumerator.Current.WriteTo(writer);
