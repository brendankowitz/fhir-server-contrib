...
public class PartialDateTime
{
    ...
    public static readonly PartialDateTime MinValue = new(DateTimeOffset.MinValue);
    public static readonly PartialDateTime MaxValue = new(DateTimeOffset.MaxValue);
    ...
    public static PartialDateTime Parse(string inputString)
    {
        ...
        if (GetIsDateTimePartSpecified(FractionCapture)) fraction = GetFractionFromDateTimeOffset(parsedDateTimeOffset);
        ...
        if (GetIsDateTimePartSpecified(InvalidTimeZoneCapture)) throw new FormatException(string.Format(Resources.DateTimeStringIsIncorrectlyFormatted, inputString));
        ...
        if (hour != null && utcOffset == null) utcOffset = TimeSpan.FromMinutes(0);
        ...
        {
            string stringValue = match.Groups[name]?.Value;
    ...
    private static string[] GenerateDateTimeOffsetFormats()
    {
        ...
        foreach (string timeZoneFormat in timeZoneFormats)
