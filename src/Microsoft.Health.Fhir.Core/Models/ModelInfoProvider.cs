...
public static class ModelInfoProvider
{
    ...
    public static FhirSpecification Version => Instance.Version;

    public static IStructureDefinitionSummaryProvider StructureDefinitionSummaryProvider => Instance.StructureDefinitionSummaryProvider;
    ...
    {
        if (_modelInfoProvider == null) throw new InvalidOperationException("Please call SetProvider before using methods on this class.");
    }
    ...
    {
        if (!string.IsNullOrEmpty(resourceName) && !IsKnownResource(resourceName)) throw new ArgumentException(string.Format(Resources.ResourceNotSupported, resourceName), parameterName);
    }
}
