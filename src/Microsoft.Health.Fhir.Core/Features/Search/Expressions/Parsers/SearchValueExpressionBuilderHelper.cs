...
internal class SearchValueExpressionBuilderHelper : ISearchValueVisitor
{
    ...
    void ISearchValueVisitor.Visit(DateTimeSearchValue dateTime)
    {
        ...
        switch (_comparator)
        {
            ...
            case SearchComparator.Ap:
                ...
                long differenceTicks = (long)((Clock.UtcNow.Ticks - Math.Max(startTicks, endTicks)) * ApproximateMultiplier);
    ...
    {
        if (_comparator != SearchComparator.Eq) throw new InvalidSearchOperationException(Core.Resources.OnlyEqualComparatorIsSupported);
    }
