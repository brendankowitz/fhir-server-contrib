...
public class ExpressionParser : IExpressionParser
{
    ...
    public IncludeExpression ParseInclude(string[] resourceTypes, string includeValue, bool isReversed, bool iterate)
    {
        ...
        if (!TrySplit(SearchSplitChar, ref valueSpan, out ReadOnlySpan<char> originalType)) throw new InvalidSearchOperationException(isReversed ? Core.Resources.RevIncludeMissingType : Core.Resources.IncludeMissingType);

        if (resourceTypes.Length == 1 && resourceTypes[0].Equals(KnownResourceTypes.DomainResource, StringComparison.OrdinalIgnoreCase)) throw new InvalidSearchOperationException(Core.Resources.IncludeCannotBeAgainstBase);
    ...
    private Expression ParseImpl(string[] resourceTypes, ReadOnlySpan<char> key, string value)
    {
        ...
        {
            if (!TrySplit(SearchSplitChar, ref key, out ReadOnlySpan<char> type)) throw new InvalidSearchOperationException(Core.Resources.ReverseChainMissingType);

            if (!TrySplit(SearchSplitChar, ref key, out ReadOnlySpan<char> refParam)) throw new InvalidSearchOperationException(Core.Resources.ReverseChainMissingReference);
