...
public class QuantitySearchValue : ISearchValue
{
    ...
    public bool Equals([AllowNull] ISearchValue other)
    {
        ...
        if (quantitytSearchValueOther == null) return false;
    ...
    public static QuantitySearchValue Parse(string s)
    {
        ...
        if (parts.Count > 3) throw new FormatException(Core.Resources.MoreThanTwoTokenSeparatorSpecified);

        decimal quantity;
        if (!decimal.TryParse(parts[0], NumberStyles.Number | NumberStyles.AllowExponent, CultureInfo.InvariantCulture, out quantity)) throw new BadRequestException(string.Format(Core.Resources.MalformedSearchValue, parts[0]));
