...
public class SearchParameterStatusManager : INotificationHandler<SearchParameterDefinitionManagerInitialized>
{
    ...
    internal async Task EnsureInitializedAsync(CancellationToken cancellationToken)
    {
        ...
        IReadOnlyCollection<ResourceSearchParameterStatus> searchParamResourceStatus = await _searchParameterStatusDataStore.GetSearchParameterStatuses(cancellationToken);
        ...
        foreach (SearchParameterInfo p in _searchParameterDefinitionManager.AllSearchParameters)
            ...
            {
                TempStatus tempStatus = EvaluateSearchParamStatus(result);
    ...
    public async Task UpdateSearchParameterStatusAsync(IReadOnlyCollection<string> searchParameterUris, SearchParameterStatus status, CancellationToken cancellationToken)
    {
        ...
        if (searchParameterUris.Count == 0) return;
        ...
        foreach (string uri in searchParameterUris)
        {
            ...
            if (parameters.TryGetValue(uri, out ResourceSearchParameterStatus existingStatus))
            {
    ...
    {
        IReadOnlyCollection<ResourceSearchParameterStatus> searchParamStatus = await _searchParameterStatusDataStore.GetSearchParameterStatuses(cancellationToken);
    ...
    {
        if (!updatedSearchParameterStatus.Any()) return;

        var updated = new List<SearchParameterInfo>();

        foreach (ResourceSearchParameterStatus paramStatus in updatedSearchParameterStatus)
            if (_searchParameterDefinitionManager.TryGetSearchParameter(paramStatus.Uri.OriginalString, out SearchParameterInfo param))
            {
                TempStatus tempStatus = EvaluateSearchParamStatus(paramStatus);
        ...
        if (updatedSearchParameterStatus.Any()) _latestSearchParams = updatedSearchParameterStatus.Select(p => p.LastUpdated).Max();
