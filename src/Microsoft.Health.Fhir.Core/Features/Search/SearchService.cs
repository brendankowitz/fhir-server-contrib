...
public abstract class SearchService : ISearchService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (before != null)
        {
            DateTimeOffset beforeOffset = before.ToDateTimeOffset(
                1,
                (year, month) => 1,
                0,
                0,
        ...
        if (searchByResourceId) queryParameters.Add(Tuple.Create(SearchParameterNames.Id, resourceId));

        if (!string.IsNullOrEmpty(continuationToken)) queryParameters.Add(Tuple.Create(KnownQueryParameterNames.ContinuationToken, continuationToken));
        ...
        {
            if (since != null) queryParameters.Add(Tuple.Create(SearchParameterNames.LastUpdated, $"ge{since}"));

            if (before != null) queryParameters.Add(Tuple.Create(SearchParameterNames.LastUpdated, $"lt{before}"));
        }

        if (count.HasValue && count > 0) queryParameters.Add(Tuple.Create(KnownQueryParameterNames.Count, count.ToString()));
        ...
        {
            ResourceWrapper resource = await _fhirDataStore.GetAsync(new ResourceKey(resourceType, resourceId), cancellationToken);

            if (resource == null) throw new ResourceNotFoundException(string.Format(Core.Resources.ResourceNotFoundById, resourceType, resourceId));
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (countOnly) searchOptions.CountOnly = true;

        SearchResult results = await SearchForReindexInternalAsync(searchOptions, searchParameterHash, cancellationToken);
