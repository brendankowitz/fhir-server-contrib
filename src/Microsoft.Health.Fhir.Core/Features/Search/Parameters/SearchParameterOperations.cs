...
public class SearchParameterOperations : ISearchParameterOperations
{
    ...
    public async Task AddSearchParameterAsync(ITypedElement searchParam, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (!supportedResult.Supported) throw new SearchParameterNotSupportedException(searchParameterInfo.Url);

            // check data store specific support for SearchParameter
            if (!_dataStoreSearchParameterValidator.ValidateSearchParameter(searchParameterInfo, out string errorMessage)) throw new SearchParameterNotSupportedException(errorMessage);
    ...
    public async Task DeleteSearchParameterAsync(RawResource searchParamResource, CancellationToken cancellationToken)
    {
        try
        {
            ...
            ITypedElement searchParam = _modelInfoProvider.ToTypedElement(searchParamResource);
            string searchParameterUrl = searchParam.GetStringScalar("url");
    ...
    public async Task UpdateSearchParameterAsync(ITypedElement searchParam, RawResource previousSearchParam, CancellationToken cancellationToken)
    {
        try
        {
            ...
            ITypedElement prevSearchParam = _modelInfoProvider.ToTypedElement(previousSearchParam);
            string prevSearchParamUrl = prevSearchParam.GetStringScalar("url");
    ...
    {
        IReadOnlyCollection<ResourceSearchParameterStatus> updatedSearchParameterStatus = await _searchParameterStatusManager.GetSearchParameterStatusUpdates(cancellationToken);
        ...
        foreach (ResourceSearchParameterStatus searchParam in updatedSearchParameterStatus.Where(p => p.Status == SearchParameterStatus.Deleted)) DeleteSearchParameter(searchParam.Uri.OriginalString);

        var paramsToAdd = new List<ITypedElement>();
        foreach (ResourceSearchParameterStatus searchParam in updatedSearchParameterStatus.Where(p => p.Status != SearchParameterStatus.Deleted))
        {
            ITypedElement searchParamResource = await GetSearchParameterByUrl(searchParam.Uri.OriginalString, cancellationToken);
            ...
            if (_searchParameterDefinitionManager.TryGetSearchParameter(searchParam.Uri.OriginalString, out SearchParameterInfo existingSearchParam))
        ...
        if (paramsToAdd.Any()) _searchParameterDefinitionManager.AddNewSearchParameters(paramsToAdd);
    ...
    private async Task<ITypedElement> GetSearchParameterByUrl(string url, CancellationToken cancellationToken)
    {
        ...
        SearchResult result = await search.Value.SearchAsync(KnownResourceTypes.SearchParameter, queryParams, cancellationToken);

        if (result.Results.Any())
        {
            if (result.Results.Count() > 1) _logger.LogWarning("More than one SearchParameter found with url {0}. This may cause unpredictable behavior.", url);
