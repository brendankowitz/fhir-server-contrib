...
internal class CapabilityStatementBuilder : ICapabilityStatementBuilder
{
    ...
    public ICapabilityStatementBuilder PopulateDefaultResourceInteractions()
    {
        foreach (string resource in _modelInfoProvider.GetResourceTypeNames())
        {
            ...
            if (string.Equals(resource, KnownResourceTypes.Parameters, StringComparison.Ordinal)) continue;
    ...
    {
        foreach (string resource in _modelInfoProvider.GetResourceTypeNames()) ApplyToResource(resource, c => c.SearchRevInclude.Clear());
    ...
    public ITypedElement Build()
    {
        ...
        string json = JsonConvert.SerializeObject(_statement, new JsonSerializerSettings
        {
    ...
    public static ICapabilityStatementBuilder Create(IModelInfoProvider modelInfoProvider, ISearchParameterDefinitionManager searchParameterDefinitionManager, IOptions<CoreFeatureConfiguration> configuration, IKnowSupportedProfiles supportedProfiles)
    {
        ...
        ListedCapabilityStatement statement = JsonConvert.DeserializeObject<ListedCapabilityStatement>(reader.ReadToEnd());

        FileVersionInfo version = ProductVersionInfo.Version;
        string versionString = $"{version.FileMajorPart}.{version.FileMinorPart}.{version.FileBuildPart}";
    ...
    private void RemoveRestInteraction(string resourceType, string interaction)
    {
        ApplyToResource(resourceType, c =>
        {
            ResourceInteractionComponent toRemove = c.Interaction.Where(x => x.Code == interaction).FirstOrDefault();
            if (toRemove != null) c.Interaction.Remove(toRemove);
        });
    }
    ...
    private ICapabilityStatementBuilder SyncSearchParams(string resourceType)
    {
        ...
        if (searchParams.Any())
        {
            ApplyToResource(resourceType, c =>
            {
                         ...
                         }))
                {
                    ...
                    if (string.Equals("_type", searchParam.Name, StringComparison.OrdinalIgnoreCase)) continue;
        ...
        ApplyToResource(resourceType, c =>
        {
            ...
            foreach (SearchParameterInfo referenceParam in searchParams.Where(x => x.Type == SearchParamType.Reference)) c.SearchInclude.Add($"{resourceType}:{referenceParam.Code}");

            if (c.SearchInclude.Any()) c.SearchInclude.Add("*");
        });

        // Add search revinclude for resource
        foreach (SearchParameterInfo referenceParam in searchParams.Where(x => x.Type == SearchParamType.Reference))
        foreach (string targetType in referenceParam.TargetResourceTypes)
    ...
    private ICapabilityStatementBuilder SyncProfile(string resourceType, bool disableCacheRefresh)
    {
        ...
        {
            IEnumerable<string> supportedProfiles = _supportedProfiles.GetSupportedProfiles(resourceType, disableCacheRefresh);
            if (supportedProfiles != null)
            {
                if (!_modelInfoProvider.Version.Equals(FhirSpecification.Stu3))
                {
                    resourceComponent.SupportedProfile.Clear();
                    foreach (string profile in supportedProfiles) resourceComponent.SupportedProfile.Add(profile);
                }
                else
                {
