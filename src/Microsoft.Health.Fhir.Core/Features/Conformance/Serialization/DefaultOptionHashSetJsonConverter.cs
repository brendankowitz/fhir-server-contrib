    ...
    public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
    {
        ...
        if (value is IDefaultOption hasDefaultOption)
        {
            ...
            if (value is IEnumerable enumerable)
            {
                ...
                if (objects.Any() && !objects.Any(x => Equals(defaultOption, x))) throw new UnsupportedConfigurationException(string.Format(Core.Resources.InvalidConfigSetting, defaultOption, string.Join(", ", objects.ToArray())));
            }
        }
