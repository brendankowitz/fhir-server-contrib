    ...
    : ConformanceProviderBase, IConfiguredConformanceProvider, INotificationHandler<RebuildCapabilityStatement>, IDisposable
{
    ...
    private SemaphoreSlim _defaultCapabilitySemaphore = new(1, 1);
    private ResourceElement _listedCapabilityStatement;
    private ResourceElement _metadata;
    private SemaphoreSlim _metadataSemaphore = new(1, 1);
    ...
    public override async Task<ResourceElement> GetCapabilityStatementOnStartup(CancellationToken cancellationToken = default)
    {
        if (_listedCapabilityStatement == null)
        {
            ...
            try
            {
                if (_listedCapabilityStatement == null)
                {
                    ...
                    {
                        foreach (IProvideCapability provider in providerFactory.Value) provider.Build(_builder);
                    }

                    foreach (Action<ListedCapabilityStatement> postConfiguration in _configurationUpdates) _builder.Apply(statement => postConfiguration(statement));
    ...
    public void ConfigureOptionalCapabilities(Action<ListedCapabilityStatement> builder)
    {
        ...
        if (_listedCapabilityStatement != null) throw new InvalidOperationException("Post capability configuration changes can no longer be applied.");
