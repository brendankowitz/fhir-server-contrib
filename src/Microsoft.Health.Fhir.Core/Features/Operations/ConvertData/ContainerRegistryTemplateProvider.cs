...
public class ContainerRegistryTemplateProvider : IConvertDataTemplateProvider, IDisposable
{
    ...
    public async Task<List<Dictionary<string, Template>>> GetTemplateCollectionAsync(ConvertDataRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!request.IsDefaultTemplateReference)
        {
            ...
            {
                string token = await _containerRegistryTokenProvider.GetTokenAsync(request.RegistryServer, cancellationToken);
        ...
        try
        {
            ITemplateCollectionProvider provider = _templateCollectionProviderFactory.CreateTemplateCollectionProvider(request.TemplateCollectionReference, accessToken);
    ...
    {
        DateTimeOffset defaultExpiration = DateTimeOffset.Now.AddMinutes(30);
        if (accessToken.StartsWith("bearer ", StringComparison.OrdinalIgnoreCase))
        {
            string jwtTokenText = accessToken.Substring(7);
