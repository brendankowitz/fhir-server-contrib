...
public abstract class PatchPayload
{
    ...
    public ResourceElement Patch(ResourceWrapper resourceToPatch)
    {
        ...
        var resource = resourceToPatch.RawResource.ToITypedElement(ModelInfoProvider.Instance);
        ...
        if (!preState.Zip(postState).All(x => x.First.path == x.Second.path && string.Equals(x.First.result?.ToString(), x.Second.result?.ToString(), StringComparison.Ordinal))) throw new RequestNotValidException(Core.Resources.PatchImmutablePropertiesIsNotValid);
