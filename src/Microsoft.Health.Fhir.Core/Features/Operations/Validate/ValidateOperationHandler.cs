...
{
    public static readonly OperationOutcomeIssue ValidationPassed = new(
    ...
    {
        if (await _authorizationService.CheckAccess(DataActions.ResourceValidate, cancellationToken) != DataActions.ResourceValidate) throw new UnauthorizedFhirActionException();

        OperationOutcomeIssue[] outcome = _profileValidator.TryValidate(request.Resource.Instance, request.Profile?.ToString());
        if (outcome.Length == 0) return new ValidateOperationResponse(ValidationPassed);
