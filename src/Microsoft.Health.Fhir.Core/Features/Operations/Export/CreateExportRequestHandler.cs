...
public class CreateExportRequestHandler : IRequestHandler<CreateExportRequest, CreateExportResponse>
{
    ...
    public async Task<CreateExportResponse> Handle(CreateExportRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await _authorizationService.CheckAccess(DataActions.Export, cancellationToken) != DataActions.Export) throw new UnauthorizedFhirActionException();
        ...
        string storageAccountConnectionHash = string.IsNullOrEmpty(_exportJobConfiguration.StorageAccountConnection) ? string.Empty : StringExtensions.ComputeHash(_exportJobConfiguration.StorageAccountConnection);
        ...
        IList<ExportJobFilter> filters = ParseFilter(request.Filters);
    ...
    private static IList<ExportJobFilter> ParseFilter(string filterString)
    {
        ...
        if (!string.IsNullOrWhiteSpace(filterString))
        {
            ...
            foreach (string filter in filterArray)
            {
                ...
                if (parameterIndex < 0 || parameterIndex == filter.Length - 1) throw new BadRequestException(string.Format(Core.Resources.TypeFilterUnparseable, filter));
                ...
                foreach (string parameter in filterParameters)
                {
                    ...
                    if (keyValue.Length != 2) throw new BadRequestException(string.Format(Core.Resources.TypeFilterUnparseable, filter));
    ...
    private ExportJobFormatConfiguration ParseFormat(string formatName, bool useContainer)
    {
        ...
        if (formatName != null)
        {
            ...
            if (formatConfiguration == null) throw new BadRequestException(string.Format(Core.Resources.ExportFormatNotFound, formatName));
        }
