...
public class ExportJobTask : IExportJobTask
{
    ...
    public async Task ExecuteAsync(ExportJobRecord exportJobRecord, WeakETag weakETag, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            string connectionHash = string.IsNullOrEmpty(_exportJobConfiguration.StorageAccountConnection) ? string.Empty : StringExtensions.ComputeHash(_exportJobConfiguration.StorageAccountConnection);

            if (string.IsNullOrEmpty(exportJobRecord.StorageAccountUri))
            {
                if (!string.Equals(exportJobRecord.StorageAccountConnectionHash, connectionHash, StringComparison.Ordinal)) throw new DestinationConnectionException("Storage account connection string was updated during an export job.", HttpStatusCode.BadRequest);
            }
            else
            {
            ...
            if (_exportJobRecord.Progress == null) _exportJobRecord.Progress = new ExportJobProgress(null, 0);
            ...
            if (_exportJobRecord.Since != null) queryParametersList.Add(Tuple.Create(KnownQueryParameterNames.LastUpdated, $"ge{_exportJobRecord.Since}"));
        ...
        CancellationToken cancellationToken)
    {
        ...
        var queryParametersList = new List<Tuple<string, string>>(sharedQueryParametersList);
        if (progress.ContinuationToken != null) queryParametersList.Add(Tuple.Create(KnownQueryParameterNames.ContinuationToken, progress.ContinuationToken));

        string[] requestedResourceTypes = _exportJobRecord.ResourceType?.Split(',');
        var filteredResources = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        if (_exportJobRecord.Filters != null)
            foreach (ExportJobFilter filter in _exportJobRecord.Filters)
        ...
        if (progress.CurrentFilter != null) await ProcessFilter(exportJobConfiguration, progress, queryParametersList, sharedQueryParametersList, anonymizer, "filter", cancellationToken);
            ...
            !filteredResources.Contains(KnownResourceTypes.Patient))
        {
            ...
            else if (_exportJobRecord.ExportType == ExportJobType.All && requestedResourceTypes != null)
            {
                ...
                foreach (string resource in requestedResourceTypes)
                    if (!filteredResources.Contains(resource))
                        resources.Add(resource);

                if (resources.Count > 0) queryParametersList.Add(Tuple.Create(KnownQueryParameterNames.Type, resources.JoinByOrSeparator()));
            }
    ...
    private async Task ProcessFilter(
        ...
        CancellationToken cancellationToken)
    {
        int index = _exportJobRecord.Filters.IndexOf(exportJobProgress.CurrentFilter);
        var filterQueryParametersList = new List<Tuple<string, string>>(queryParametersList);
        foreach (Tuple<string, string> param in exportJobProgress.CurrentFilter.Parameters) filterQueryParametersList.Add(param);
        ...
        while (progress.ContinuationToken != null || progress.Page == 0)
        {
            ...
            if (_exportJobRecord.ExportType == ExportJobType.Patient || _exportJobRecord.ExportType == ExportJobType.Group)
            {
                ...
                foreach (SearchResultEntry result in searchResult.Results)
                {
                    ...
                    if (progress.SubSearch == null) progress.NewSubSearch(result.Resource.ResourceId);
        ...
        string batchIdPrefix = "")
    {
        ...
        if (progress.CurrentFilter != null) await ProcessFilterForCompartment(exportJobConfiguration, progress, queryParametersList, anonymizer, batchIdPrefix + "-filter", cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!replacedContinuationToken) queryParametersList.Add(Tuple.Create(KnownQueryParameterNames.ContinuationToken, progress.ContinuationToken));
