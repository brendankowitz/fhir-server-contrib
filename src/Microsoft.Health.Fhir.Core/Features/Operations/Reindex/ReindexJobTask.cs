...
public sealed class ReindexJobTask : IReindexJobTask, IDisposable
{
    ...
    public async Task ExecuteAsync(ReindexJobRecord reindexJobRecord, WeakETag weakETag, CancellationToken cancellationToken)
    {
        ...
        if (_reindexJobRecord != null) throw new NotSupportedException($"{nameof(ReindexJobTask)} can work only on one {nameof(reindexJobRecord)}. Please create new {nameof(ReindexJobTask)} to process this instance of {nameof(reindexJobRecord)}");
        ...
        IFhirRequestContext originalRequestContext = _contextAccessor.RequestContext;
        ...
        try
        {
            ...
            if (reindexJobRecord.TargetDataStoreUsagePercentage != null &&
                ...
                {
                    int? provisionedCapacity = await store.Value.GetProvisionedDataStoreCapacityAsync(_cancellationToken);
    ...
    private async Task<bool> TryPopulateNewJobFields()
    {
        ...
        IEnumerable<SearchParameterInfo> possibleNotYetIndexedParams = _supportedSearchParameterDefinitionManager.GetSearchParametersRequiringReindexing();
        ...
        {
            foreach (SearchParameterInfo searchParam in possibleNotYetIndexedParams)
            {
                ICollection<string> searchParamResourceTypes = GetDerivedResourceTypes(searchParam.BaseResourceTypes);
                IEnumerable<string> matchingResourceTypes = searchParamResourceTypes.Intersect(_reindexJobRecord.TargetResourceTypes);
        ...
        else
        {
            ...
            foreach (SearchParameterInfo param in notYetIndexedParams)
            {
                ICollection<string> searchParamResourceTypes = GetDerivedResourceTypes(param.BaseResourceTypes);
        ...
        foreach (string resource in resourceList) _reindexJobRecord.Resources.Add(resource);

        // save the list of search parameters to the reindexjob document
        foreach (string searchParams in notYetIndexedParams.Select(p => p.Url.OriginalString)) _reindexJobRecord.SearchParams.Add(searchParams);
        ...
        foreach (string resourceType in _reindexJobRecord.Resources)
            ...
            {
                var query = new ReindexJobQueryStatus(resourceType, null)
                {
    ...
    private async Task ProcessJob()
    {
                   ...
                   q.Status == OperationStatus.Running).Any())
        {
            if (_reindexJobRecord.QueryList.Keys.Where(q => q.Status == OperationStatus.Queued).Any())
            {
                ...
                ReindexJobQueryStatus query = _reindexJobRecord.QueryList.Keys.Where(q => q.Status == OperationStatus.Queued).OrderBy(q => q.LastModified).FirstOrDefault();
                var queryTokensSource = new CancellationTokenSource();
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
            ...
            IEnumerable<ReindexJobQueryStatus> staleQueries = _reindexJobRecord.QueryList.Keys.Where(
                q => q.Status == OperationStatus.Running && q.LastModified < Clock.UtcNow - _reindexJobConfiguration.JobHeartbeatTimeoutThreshold);
            foreach (ReindexJobQueryStatus staleQuery in staleQueries)
            {
                await _jobSemaphore.WaitAsync(_cancellationToken);
                try
                {
                    // if this query has a created task, cancel it
                    if (queryCancellationTokens.TryGetValue(staleQuery, out CancellationTokenSource tokenSource))
                        try
                        {
            ...
            double averageDbConsumption = _throttleController.UpdateDatastoreUsage();
            _logger.LogInformation($"Reindex avaerage DB consumption: {averageDbConsumption}");
            int throttleDelayTime = _throttleController.GetThrottleBasedDelay();
            ...
            {
                Task<ReindexJobQueryStatus>[] taskArray = queryTasks.ToArray();
                Task.WaitAny(taskArray, _cancellationToken);
                Task<ReindexJobQueryStatus>[] finishedTasks = queryTasks.Where(t => t.IsCompleted).ToArray();
                foreach (Task<ReindexJobQueryStatus> finishedTask in finishedTasks)
                {
            ...
            try
            {
                ...
                {
                    ReindexJobWrapper wrapper = await store.Value.GetReindexJobByIdAsync(_reindexJobRecord.Id, _cancellationToken);
            ...
            {
                foreach (CancellationTokenSource tokenSource in queryCancellationTokens.Values) tokenSource.Cancel(false);
    ...
    private async Task<ReindexJobQueryStatus> ProcessQueryAsync(ReindexJobQueryStatus query, CancellationToken cancellationToken)
    {
        try
        {
            ...
            try
            {
                ...
                {
                    string encodedContinuationToken = ContinuationTokenConverter.Encode(results.ContinuationToken);
            ...
            if (!_cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        ReindexJobQueryStatus queryStatusToRemove = _reindexJobRecord.QueryList.Keys.Where(q => q.Status == OperationStatus.Completed).OrderBy(q => q.LastModified).FirstOrDefault();
                        _reindexJobRecord.QueryList.TryRemove(queryStatusToRemove, out byte removedByte);
                    }
    ...
    private async Task<SearchResult> ExecuteReindexQueryAsync(ReindexJobQueryStatus queryStatus, bool countOnly, CancellationToken cancellationToken)
    {
        ...
        if (queryStatus.ContinuationToken != null) queryParametersList.Add(Tuple.Create(KnownQueryParameterNames.ContinuationToken, queryStatus.ContinuationToken));

        if (!_reindexJobRecord.ResourceTypeSearchParameterHashMap.TryGetValue(queryStatus.ResourceType, out string searchParameterHash)) searchParameterHash = string.Empty;
        ...
        using (IScoped<ISearchService> searchService = _searchServiceFactory())
        {
            ...
            {
                string message = $"Error running reindex query for resource type {queryStatus.ResourceType}.";
    ...
    private async Task UpdateJobAsync()
    {
        ...
        {
            ReindexJobWrapper wrapper = await store.Value.UpdateReindexJobAsync(_reindexJobRecord, _weakETag, _cancellationToken);
            _weakETag = wrapper.ETag;
        }
    }
    ...
    private async Task CalculateAndSetTotalAndResourceCounts()
    {
        ...
        foreach (string resourceType in _reindexJobRecord.Resources)
        {
            var queryForCount = new ReindexJobQueryStatus(resourceType, null)
            {
                LastModified = Clock.UtcNow,
                Status = OperationStatus.Queued
            };
            ...
            SearchResult countOnlyResults = await ExecuteReindexQueryAsync(queryForCount, true, _cancellationToken);
    ...
    private async Task UpdateParametersAndCompleteJob()
    {
        ...
        foreach (string searchParam in _reindexJobRecord.SearchParams)
        {
            SearchParameterInfo searchParamInfo = _supportedSearchParameterDefinitionManager.GetSearchParameter(searchParam);
            if (reindexedResourcesSet.IsSupersetOf(searchParamInfo.BaseResourceTypes)) fullyIndexedParamUris.Add(searchParam);
        }
    ...
    private ICollection<string> GetDerivedResourceTypes(IReadOnlyCollection<string> resourceTypes)
    {
        ...
        foreach (string baseResourceType in resourceTypes)
        {
