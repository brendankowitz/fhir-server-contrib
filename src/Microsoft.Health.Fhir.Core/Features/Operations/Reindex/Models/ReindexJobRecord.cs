...
public class ReindexJobRecord : JobRecord
{
        ...
        ushort? targetDataStoreUsagePercentage = null)
    {
        ...
        if (maxiumumConcurrency < MinMaximumConcurrency || maxiumumConcurrency > MaxMaximumConcurrency)
            throw new BadRequestException(string.Format(Core.Resources.InvalidReIndexParameterValue, nameof(MaximumConcurrency), MinMaximumConcurrency.ToString(), MaxMaximumConcurrency.ToString()));
        else
        ...
        if (maxResourcesPerQuery < MinMaximumNumberOfResourcesPerQuery || maxResourcesPerQuery > MaxMaximumNumberOfResourcesPerQuery)
            throw new BadRequestException(string.Format(Core.Resources.InvalidReIndexParameterValue, nameof(MaximumNumberOfResourcesPerQuery), MinMaximumNumberOfResourcesPerQuery.ToString(), MaxMaximumNumberOfResourcesPerQuery.ToString()));
        else
        ...
        if (queryDelayIntervalInMilliseconds < MinQueryDelayIntervalInMilliseconds || queryDelayIntervalInMilliseconds > MaxQueryDelayIntervalInMilliseconds)
            throw new BadRequestException(string.Format(Core.Resources.InvalidReIndexParameterValue, nameof(QueryDelayIntervalInMilliseconds), MinQueryDelayIntervalInMilliseconds.ToString(), MaxQueryDelayIntervalInMilliseconds.ToString()));
        else
        ...
        if (targetDataStoreUsagePercentage < MinTargetDataStoreUsagePercentage || targetDataStoreUsagePercentage > MaxTargetDataStoreUsagePercentage)
            throw new BadRequestException(string.Format(Core.Resources.InvalidReIndexParameterValue, nameof(TargetDataStoreUsagePercentage), MinTargetDataStoreUsagePercentage.ToString(), MaxTargetDataStoreUsagePercentage.ToString()));
        else
            TargetDataStoreUsagePercentage = targetDataStoreUsagePercentage;

        // check for TargetResourceTypes boundary
        foreach (string type in targetResourceTypes) ModelInfoProvider.EnsureValidResourceType(type, nameof(type));
    ...
    public ConcurrentDictionary<ReindexJobQueryStatus, byte> QueryList { get; private set; } = new();

    [JsonProperty(JobRecordProperties.ResourceCounts)]
    public ConcurrentDictionary<string, int> ResourceCounts { get; private set; } = new();
    ...
    [JsonIgnore] public string ResourceList => string.Join(",", Resources);

    [JsonIgnore] public string SearchParamList => string.Join(",", SearchParams);

    [JsonIgnore] public string TargetResourceTypeList => string.Join(",", TargetResourceTypes);
}
