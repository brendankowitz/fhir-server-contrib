...
public class CreateReindexRequestHandler : IRequestHandler<CreateReindexRequest, CreateReindexResponse>
{
    ...
    public async Task<CreateReindexResponse> Handle(CreateReindexRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await _authorizationService.CheckAccess(DataActions.Reindex, cancellationToken) != DataActions.Reindex) throw new UnauthorizedFhirActionException();

        (bool activeReindexJobs, string reindexJobId) = await _fhirOperationDataStore.CheckActiveReindexJobsAsync(cancellationToken);
        if (activeReindexJobs) throw new JobConflictException(string.Format(Core.Resources.OnlyOneResourceJobAllowed, reindexJobId));
        ...
        ReindexJobWrapper outcome = await _fhirOperationDataStore.CreateReindexJobAsync(jobRecord, cancellationToken);
