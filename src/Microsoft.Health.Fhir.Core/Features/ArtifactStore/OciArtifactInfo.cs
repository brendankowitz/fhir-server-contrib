...
public class OciArtifactInfo
{
    ...
    public bool ContainsOciImage(string server, string name, string digest)
    {
        ...
        if (string.IsNullOrEmpty(LoginServer) || !string.Equals(LoginServer, server, StringComparison.OrdinalIgnoreCase)) return false;

        // Compare imageName if ImageName is provided in ArtifactInfo.
        if (!string.IsNullOrEmpty(ImageName) && !string.Equals(ImageName, name, StringComparison.OrdinalIgnoreCase)) return false;

        // Compare digest if Digest is provided in ArtifactInfo.
        if (!string.IsNullOrEmpty(Digest) && !string.Equals(Digest, digest, StringComparison.OrdinalIgnoreCase)) return false;
