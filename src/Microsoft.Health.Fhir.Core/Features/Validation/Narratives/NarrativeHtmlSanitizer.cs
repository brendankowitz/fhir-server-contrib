...
public class NarrativeHtmlSanitizer : INarrativeHtmlSanitizer
{
    ...
    public IEnumerable<string> Validate(string html)
    {
        ...
        using (_logger.BeginTimedScope($"{nameof(NarrativeHtmlSanitizer)}.{nameof(Validate)}"))
        {
            ...
            IHtmlDocument dom = parser.ParseDocument(string.Format(HtmlTemplate, html));

            // Report parsing errors
            if (errors.Any())
            {
                foreach (HtmlErrorEvent error in errors.Where(x => RaiseErrorTypes.Contains((HtmlParseError)x.Code))) yield return string.Format(Core.Resources.IllegalHtmlParsingError, error.Message, error.Position.Line, error.Position.Column);

                yield break;
            }

            IHtmlBodyElement htmlBodyElement = dom.Children.OfType<IHtmlHtmlElement>().FirstOrDefault()
            ...
            foreach (string htmlError in invalidHtml) yield return htmlError;
        }
    }
    ...
    public string Sanitize(string html)
    {
        ...
        using (_logger.BeginTimedScope($"{nameof(NarrativeHtmlSanitizer)}.{nameof(Sanitize)}"))
        {
            ...
            using (IHtmlDocument dom = parser.ParseDocument(string.Format(HtmlTemplate, html)))
            {
                IHtmlDivElement containerDiv = dom.Body.Children.OfType<IHtmlDivElement>().FirstOrDefault();
                if (containerDiv == null) return string.Empty;
        ...
        Action<IElement, IAttr> onInvalidAttr)
    {
        ...
        {
            if (!AllowedElements.Contains(element.NodeName)) onInvalidElement(element);
    ...
    private static void ValidateAttributes(IElement element, Action<IElement, IAttr> onInvalidAttr)
    {
        ...
        {
            if (!AllowedAttributes.Contains(attr.Name)) onInvalidAttr(element, attr);
