...
public class CompartmentIndices
{
    ...
    internal CompartmentIndices(IDictionary<string, IReadOnlyCollection<string>> compartmentTypeToResourceIds)
    {
        ...
        if (compartmentTypeToResourceIds.TryGetValue(KnownCompartmentTypes.Device, out compartmentEntry)) DeviceCompartmentEntry = compartmentEntry;

        if (compartmentTypeToResourceIds.TryGetValue(KnownCompartmentTypes.Encounter, out compartmentEntry)) EncounterCompartmentEntry = compartmentEntry;

        if (compartmentTypeToResourceIds.TryGetValue(KnownCompartmentTypes.Patient, out compartmentEntry)) PatientCompartmentEntry = compartmentEntry;

        if (compartmentTypeToResourceIds.TryGetValue(KnownCompartmentTypes.Practitioner, out compartmentEntry)) PractitionerCompartmentEntry = compartmentEntry;

        if (compartmentTypeToResourceIds.TryGetValue(KnownCompartmentTypes.RelatedPerson, out compartmentEntry)) RelatedPersonCompartmentEntry = compartmentEntry;
    }
