...
public class ResourceDeserializer : IResourceDeserializer
{
    ...
    internal ResourceElement DeserializeRaw(RawResource rawResource, string version, DateTimeOffset lastModified)
    {
        ...
        if (!_deserializers.TryGetValue(rawResource.Format, out Func<string, string, DateTimeOffset, ResourceElement> deserializer)) throw new NotSupportedException();
    ...
    internal ResourceElement DeserializeRawResourceElement(RawResourceElement rawResourceElement)
    {
        ...
        if (!_deserializers.TryGetValue(rawResourceElement.Format, out Func<string, string, DateTimeOffset, ResourceElement> deserializer)) throw new NotSupportedException();
