...
public class ResourceWrapperFactory : IResourceWrapperFactory
{
    ...
    {
        ResourceElement resourceElement = _resourceDeserializer.Deserialize(resourceWrapper);
    ...
    private static void ExtractMinAndMaxValues(IReadOnlyCollection<SearchIndexEntry> searchIndices)
    {
        ...
        foreach (SearchIndexEntry currentEntry in searchIndices)
        {
            ...
            if (currentEntry.SearchParameter.SortStatus == SortParameterStatus.Disabled) continue;

            if (minValues.TryGetValue(currentEntry.SearchParameter.Url, out ISupportSortSearchValue existingMinValue))
            {
                if (currentValue.CompareTo(existingMinValue, ComparisonRange.Min) < 0) minValues[currentEntry.SearchParameter.Url] = currentValue;
            }
            else
            {
            ...
            {
                if (currentValue.CompareTo(existingMaxValue, ComparisonRange.Max) > 0) maxValues[currentEntry.SearchParameter.Url] = currentValue;
            }
            else
            {
        ...
        foreach (KeyValuePair<Uri, ISupportSortSearchValue> kvp in minValues) kvp.Value.IsMin = true;

        foreach (KeyValuePair<Uri, ISupportSortSearchValue> kvp in maxValues) kvp.Value.IsMax = true;
    }
}
