...
public class SearchParameterDefinitionManager : ISearchParameterDefinitionManager, IHostedService, INotificationHandler<SearchParametersUpdatedNotification>, INotificationHandler<StorageInitializedNotification>
{
    ...
    {
        BundleWrapper bundle = SearchParameterDefinitionBuilder.ReadEmbeddedSearchParameters("search-parameters.json", _modelInfoProvider);
    ...
    public void DeleteSearchParameter(string url, bool calculateHash = true)
    {
        ...
        if (!UrlLookup.TryRemove(url, out searchParameterInfo)) throw new ResourceNotFoundException(string.Format(Core.Resources.CustomSearchParameterNotfound, url));
    ...
    private async Task LoadSearchParamsFromDataStore(CancellationToken cancellationToken)
    {
        ...
        do
        {
            ...
            if (continuationToken != null) searchOptions.ContinuationToken = continuationToken;

            SearchResult result = await search.Value.SearchAsync(searchOptions, cancellationToken);
            ...
            if (result?.Results != null && result.Results.Any())
            {
                ...
                foreach (ITypedElement searchParam in searchParams)
                    try
                    {
                        SearchParameterDefinitionBuilder.Build(
