...
internal static class SearchParameterDefinitionBuilder
{
    ...
    private static SearchParameterInfo GetOrCreateSearchParameterInfo(SearchParameterWrapper searchParameter, IDictionary<string, SearchParameterInfo> uriDictionary)
    {
        ...
        if (uriDictionary.TryGetValue(searchParameter.Url, out SearchParameterInfo spi)) return spi;
        ...
        IModelInfoProvider modelInfoProvider)
    {
        ...
        foreach (SearchParameterWrapper searchParameter in searchParameters)
        {
            if (searchParameter == null) continue;
            ...
            {
                if (modelInfoProvider.Version == FhirSpecification.R5 && _missingExpressionsInR5.Contains(new Uri(searchParameter.Url))) continue;
