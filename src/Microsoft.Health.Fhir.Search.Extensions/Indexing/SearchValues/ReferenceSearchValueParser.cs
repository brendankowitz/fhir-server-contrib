...
namespace Microsoft.Health.Fhir.Search.Extensions.Indexing.SearchValues
{
    ...
    public class ReferenceSearchValueParser : IReferenceSearchValueParser
    {
        ...
        private static readonly string[] SupportedSchemes = new[] { Uri.UriSchemeHttps, Uri.UriSchemeHttp };
        ...
        public ReferenceSearchValue Parse(string s)
        {
            ...
            if (match.Success)
            {
                ...
                if (!string.IsNullOrEmpty(resourceTypeInString) && !_fhirSchema.ResourceTypeNames.Contains(resourceTypeInString))
                {
                    throw new ArgumentException(string.Format(Resources.ResourceNotSupported, resourceTypeInString), resourceTypeInString);
                }
