...
namespace Microsoft.Health.Fhir.Search.Extensions.Indexing.SearchValues
{
    ...
    public class QuantitySearchValue : ISearchValue
    {
        ...
        public static QuantitySearchValue Parse(string s)
        {
            ...
            if (parts.Count > 3)
            {
                throw new FormatException(Resources.MoreThanTwoTokenSeparatorSpecified);
            }

            decimal quantity;
            if (!decimal.TryParse(parts[0], NumberStyles.Number | NumberStyles.AllowExponent, CultureInfo.InvariantCulture, out quantity))
            {
                throw new BadRequestException(string.Format(Resources.MalformedSearchValue, parts[0]));
            }
