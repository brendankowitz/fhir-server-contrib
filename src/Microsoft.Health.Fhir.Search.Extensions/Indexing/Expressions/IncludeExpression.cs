...
public class IncludeExpression : Expression
{
    ...
    public IncludeExpression(string[] resourceTypes, SearchParameterInfo referenceSearchParameter, string sourceResourceType, string targetResourceType, IEnumerable<string> referencedTypes, bool wildCard, bool reversed, bool iterate)
    {
        ...
        if (!wildCard) EnsureArg.IsNotNull(referenceSearchParameter, nameof(referenceSearchParameter));

        if (reversed) EnsureArg.IsNotNull(sourceResourceType, nameof(sourceResourceType));
    ...
    /// </summary>
    [SuppressMessage("Performance", "CA1819:Properties should not return arrays", Justification = "Array property")]
    ...
    {
        string targetType = TargetResourceType != null ? $":{TargetResourceType}" : string.Empty;
        string iterate = Iterate ? " Iterate" : string.Empty;
        string reversed = Reversed ? "Reversed " : string.Empty;
        string wildcard = WildCard ? " Wildcard" : string.Empty;
        string paramName = ReferenceSearchParameter != null ? $" {ReferenceSearchParameter.Code}" : string.Empty;
    ...
    private IReadOnlyCollection<string> GetRequiredResources()
    {
        if (Reversed)
        {
            ...
            else if (WildCard) return ReferencedTypes;
    ...
    public override void AddValueInsensitiveHashCode(ref HashCode hashCode)
    {
        ...
        foreach (string resourceType in ResourceTypes) hashCode.Add(resourceType);
