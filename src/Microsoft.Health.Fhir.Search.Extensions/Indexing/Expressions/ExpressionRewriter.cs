...
public abstract class ExpressionRewriter<TContext> : IExpressionVisitor<TContext, Expression>
{
    ...
    {
        Expression visitedExpression = expression.Expression.AcceptVisitor(this, context);
        if (ReferenceEquals(visitedExpression, expression.Expression)) return expression;
        ...
        where TExpression : Expression
    {
        ...
        {
            TExpression argument = inputArray[index];
            var rewrittenArgument = (TExpression)argument.AcceptVisitor(this, context);

            if (!ReferenceEquals(rewrittenArgument, argument)) EnsureAllocatedAndPopulated(ref outputArray, inputArray, index);

            if (outputArray != null) outputArray[index] = rewrittenArgument;
        }
        ...
        where TExpression : Expression
    {
        if (destination == null)
        {
            ...
            for (int j = 0; j < count; j++) destination[j] = source[j];
        }
    }
...
#pragma warning restore CA1002 // Do not expose generic lists
        where TExpression : Expression
    {
        if (destination == null)
        {
            ...
            for (int j = 0; j < count; j++) destination.Add(source[j]);
        }
    }
...
#pragma warning restore CA1002 // Do not expose generic lists
        ...
        where TDestination : TSource
    {
        if (destination == null)
        {
            ...
            for (int j = 0; j < count; j++) destination.Add((TDestination)source[j]);
        }
    }
}
