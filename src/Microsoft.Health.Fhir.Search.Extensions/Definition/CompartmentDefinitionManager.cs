...
using CompartmentType = Microsoft.Health.Fhir.Extensions.ValueSets.CompartmentType;
...
namespace Microsoft.Health.Fhir.Search.Extensions.Definition
{
    ...
    public class CompartmentDefinitionManager : ICompartmentDefinitionManager
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            var bundle = await JsonSourceNodeFactory.Parse<BundleJsonNode>(stream);
            Build(bundle);
        }
        ...
        private static Dictionary<CompartmentType, (CompartmentType Code, Uri Url, IList<(string Resource, IList<string> Params)> Resources)> ValidateAndGetCompartmentDict(BundleJsonNode bundle)
        {
            ...
            for (int entryIndex = 0; entryIndex < entries.Count; entryIndex++)
            {
                ...
                var sourceNode = JsonSourceNodeFactory.Create(entry.Resource);
                var compartment = sourceNode.ToTypedElement(InstanceInferredStructureDefinitionSummaryProvider.CreateFrom(sourceNode));
