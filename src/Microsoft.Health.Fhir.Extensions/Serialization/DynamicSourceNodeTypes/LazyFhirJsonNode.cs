...
public class LazyFhirJsonNode : ISourceNode, IResourceTypeSupplier, IAnnotated, IExceptionSource
{
    ...
    private readonly Lazy<JProperty> _resourceTypeProperty;
    ...
    {
        if (_jsonObject == null || _jsonObject.HasValues == false) return Enumerable.Empty<ISourceNode>();
        ...
        if (_children == null)
        {
            ...
            ILookup<string, JProperty> children = _jsonObject.Children<JProperty>().ToLookup(jp => DeriveMainName(jp));

            var processed = new HashSet<string>();

            JProperty resourceTypeChild = _resourceTypeProperty.Value;

            foreach (IGrouping<string, JProperty> child in children)
            {
                ...
                if (child.Key == "fhir_comments") continue; // ignore pre-DSTU2 Json comments

                processed.Add(child.Key);

                IGrouping<string, JProperty> innerChild = child;
    ...
    {
        foreach (object replacement in replacements) _jsonObject.Merge(JObject.FromObject(replacement), _jsonMergeSettings);
    ...
    private IEnumerable<LazyFhirJsonNode> EnumerateElement(string name, JProperty main, JProperty shadow)
    {
        ...
        // continue parsing.
        IList<JToken> mains = MakeList(main, out bool wasArrayMain);
        IList<JToken> shadows = MakeList(shadow, out bool wasArrayShadow);
        ...
        {
            LazyFhirJsonNode result = Build(name, At(mains, index), At(shadows, index), isArrayElement, index);
            if (result != null) yield return result;
        }

        JToken At(IList<JToken> list, int i)
        {
    ...
    private LazyFhirJsonNode Build(string name, JToken main, JToken shadow, bool isArrayElement, int index)
    {
        ...
        if (main?.Type == JTokenType.Null && shadow?.Type == JTokenType.Null)
            ...
            return null;
        else if (main == null && shadow?.Type == JTokenType.Null) return null;
        ...
        {
            if (v.Value is string s && string.IsNullOrWhiteSpace(s)) return null;
    ...
    {
        (int lineNumber, int linePosition) = GetPosition(node);
        ExceptionHandler.NotifyOrThrow(this, ExceptionNotification.Error(Error.Format("Parser: " + message, lineNumber, linePosition)));
    }

    private static (int lineNumber, int linePosition) GetPosition(JToken node)
    {
        return node is IJsonLineInfo jli ? (jli.LineNumber, jli.LinePosition) : (-1, -1);
    }
    ...
    private static JProperty GetResourceTypePropertyFromObject(JObject o, string name = null)
    {
        ...
        if (jProperty != null)
        {
            if (jProperty.Value.Type != JTokenType.String || !(name != "instance")) return null;
